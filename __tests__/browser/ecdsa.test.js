...
const ECDSA = require('../../src/browser/ECDSA');
...
describe('ECDSA', () => {
  ...
  it('Sign and fail at verifing wrong message', () => {
    const privateKey = Buffer.alloc(32);
    privateKey.fill(1);
    const publicKey = getPublic(privateKey);
    const msg = createHash('sha256').update('test').digest();
  ...
  it('Reject on signing with invalid key', () => {
    const msg = createHash('sha256').update('test').digest();
    ...
    expect(() => {
      try {
        console.log(ECDSA.sign(k192, msg));
      } catch (err) {
        throw err;
      }
    }).toThrow();
    expect(() => {
      try {
        console.log(ECDSA.sign(k384, msg));
      } catch (err) {
        throw err;
      }
    }).toThrow();
  ...
  it('Reject on when verifying with invalid key ', () => {
    const privateKey = Buffer.alloc(32);
    privateKey.fill(1);
    const msg = createHash('sha256').update('test').digest();
    const signature = ECDSA.sign(privateKey, msg);
  ...
  it('Reject on when verifying invalid signature ', () => {
    const privateKey = Buffer.alloc(32);
    privateKey.fill(1);
    const publicKey = getPublic(privateKey);
    const msg = createHash('sha256').update('test').digest();
  ...
  it('Sign and verify messages less than 32 bytes', () => {
    const privateKey = Buffer.alloc(32);
    privateKey.fill(1);
    const publicKey = getPublic(privateKey);
  ...
  it('Fail at signing and verifing messages longer than 32 bytes', () => {
    const privateKey = Buffer.alloc(32);
    privateKey.fill(1);
    const publicKey = getPublic(privateKey);
