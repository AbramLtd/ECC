...
int derive(const uint8_t* privkey_a, const uint8_t* pubkey_b, uint8_t* shared) {
  ...
  res = EC_KEY_set_public_key_affine_coordinates(peerkey, peerkey_bn_x,
                                                 peerkey_bn_y);
  CHECK(res == 1);
...
NAN_METHOD(Derive) {
  ...
  if (privkey_a == NULL || privkey_a_len != PRIVKEY_SIZE || pubkey_b == NULL ||
      pubkey_b_len != PUBKEY_SIZE || pubkey_b[0] != 4) {
    return Nan::ThrowError("Bad input");
  }
...
NAN_MODULE_INIT(InitAll) {
  Nan::Set(
      target, Nan::New<String>("derive").ToLocalChecked(),
